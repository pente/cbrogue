...
*/
void SdlSvgset_undoPremultipliedAlpha(SDL_Surface *surface)
{
    int x, y;
					 ...
					 RsvgHandle *svg)
{
    ...
    SdlSvgset_undoPremultipliedAlpha(surface);
			      ...
			      unsigned glyph, int frame, SDL_Color color)
{
	...
	* anim->frame_count / MAX_FRAME_COUNT;
    source = anim->frame[frame_index];
    if (source == NULL)
    {
	return NULL;
    }

    surface = SDL_ConvertSurface(source, source->format, 0);
    if (surface == NULL)
    {
	return NULL;
    }
